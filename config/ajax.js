const constant= {
  baseUrl: 'https://tcal.pipifit.com/shelf/',   //ajax基本url
  timeout: 10000,                //请求时长10s
  shopcart: 'shopcart',    //购物车数据展示、购物车商品添加、商品数量加减
  ...
  return new Promise((resolve, reject) => {
    return getNetworkType().then()     //首先检查网络状态
      .then(res => { 
        ...
        if (isToken && !token) {    //需要验证token 如果token不存在，重新登录拿token
        ...
        wx.request({
          ...
          method: method,
          header: isToken ? Object.assign(header, { token }) : header,
          success(res) {
